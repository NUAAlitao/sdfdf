...
public class Test {
	...
	public static void main(String[] args){
		...
		try {
			fileReader = new FileReader(new File("students"));
		...
		loginFrame.btlogin.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				String name = null;
				String pwd = null;
				name = loginFrame.tfname.getText().trim();
				pwd = loginFrame.tfpwd.getText().trim();
				if(students.get(name) != null){
					if(students.get(name).equals(pwd)){
						StudentManager.table = name;
						loginFrame.frame.setVisible(false);
						start();
					} else {
						JOptionPane.showMessageDialog(null, "登录失败\n用户名或密码错误!", "登录", JOptionPane.INFORMATION_MESSAGE);
					}
				} else {
					JOptionPane.showMessageDialog(null, "登录失败\n该用户不存在!", "登录", JOptionPane.INFORMATION_MESSAGE);
				}
				
			}
		});
		loginFrame.btregister.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				FileWriter file = null;
				try {
					file = new FileWriter(new File("students"), true);
					String name = loginFrame.tfname.getText().trim();
					String pwd = loginFrame.tfpwd.getText().trim();
					if(students.get(name) != null){
						JOptionPane.showMessageDialog(null, "注册失败！\n该用户已经存在！", "注册用户", JOptionPane.INFORMATION_MESSAGE);
					} else {
						students.put(name, pwd);
						file.write("\n" +name + " " + pwd);
						JOptionPane.showMessageDialog(null, "注册成功！", "注册用户", JOptionPane.INFORMATION_MESSAGE);
					}
				...
				selectFrame.btOK.addActionListener(new ActionListener() {				
					...
					public void actionPerformed(ActionEvent e) {
						...
						if(sqlselect.isEmpty()){
							sqlselect = "select * from "+studentManager.table+" ";
						}else{
							sqlselect = "select " + sqlselect + " from "+studentManager.table+" ";
						}
